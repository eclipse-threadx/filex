...
Step7: Terminate the new thread to simulate poweroff when update the data between undo log generated and redo log not generated.  
...
void    filex_fault_tolerant_write_large_data_interrupt_test_application_define(void *first_unused_memory);
...
ULONG       data_value;
    ...
    printf("FileX Test:   Fault Tolerant Write Large Data Interrupt test.........");
    ...
    for (i = 0; i < TEST_COUNT; i ++)
    {
        ...
        if ((status != FX_SUCCESS) || (data_write_interrupt != FX_TRUE) || (error_couter))
        {
...
{           
    ...
    /* Interrupt the data write operation after record the undo log.  */
