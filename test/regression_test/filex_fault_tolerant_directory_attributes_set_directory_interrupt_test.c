...
Check directory interrupt for fx_directory_attributes_set():  
Step1: Format and open the media; 
Step2: Enable fault tolerant feature;     
Step3: Create directory "TEST_DIR"; 
Step4: Check the attributes for "TEST_DIR" directory;       
Step5: Create a new thread to set the attributes as FX_DIRECTORY | FX_ARCHIVE | FX_SYSTEM | FX_READ_ONLY | FX_HIDDEN for "TEST_DIR" directory;
...
Step9: Check the attributes for "TEST_DIR" directory.
*/
...
void    filex_fault_tolerant_directory_attributes_set_directory_interrupt_test_application_define(void *first_unused_memory);
...
UINT        i;
    ...
    printf("FileX Test:   Fault Tolerant Dir Attrib Set DIR Interrupt Test.......");       
    ...
    for (i = 0; i < TEST_COUNT; i ++)
    {
        ...
        return_if_fail( (status == FX_SUCCESS) && (attributes == FX_DIRECTORY));
        ...
        /* Check the attributes read status.  */
        return_if_fail((status == FX_SUCCESS) && (attributes == (FX_ARCHIVE | FX_READ_ONLY | FX_DIRECTORY | FX_SYSTEM | FX_HIDDEN)))

        /* Close the media.  */
        status =  fx_media_close(&ram_disk);

        /* Determine if the test was successful.  */
        return_if_fail((status == FX_SUCCESS) && (directory_write_interrupt == FX_TRUE) && (error_couter == 0));
...
#endif
{
    ...
    fx_directory_attributes_set(&ram_disk, "TEST_DIR", FX_DIRECTORY | FX_ARCHIVE | FX_SYSTEM | FX_READ_ONLY | FX_HIDDEN);  
}
