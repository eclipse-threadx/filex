...
Step7: Terminate the new thread to simulate poweroff when write the directory after record the redo log.  
...
void    filex_fault_tolerant_write_large_data_directory_interrupt_test_application_define(void *first_unused_memory);
...
ULONG       data_value;
    ...
    printf("FileX Test:   Fault Tolerant Write Large Data Dir Interrupt test.....");
...
static UINT my_driver_write(FX_MEDIA *media_ptr, UINT sector_type, UCHAR *block_ptr, UINT *operation_ptr)
{
    ...
    /* Interrupt the Direcotry write operation after record the redo log.  */
