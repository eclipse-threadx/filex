...
Check directory interrupt for fx_file_write() :                  
...
Step7: Terminate the new thread to simulate poweroff when update the directory after redo log generated.  
Step8: Open the media; 
Step9: Enable fault tolerant feature to cover the data(redo operation);    
...
void    filex_fault_tolerant_file_write_directory_interrupt_test_application_define(void *first_unused_memory);
...
ULONG       actual;   
    ...
    printf("FileX Test:   Fault Tolerant File Write DIR Interrupt Test...........");
    ...
    for (i = 0; i < TEST_COUNT; i ++)
    {
        ...
        /* Check the file read status.  */  
        if ((status != FX_SUCCESS) || (actual != data_size))   
        {
...
UINT        index;
    ...
    for (write_index = 0; write_index < write_buffer_size; write_index ++)
    {
        ...
        /* Store the write data to compare the new data after Data write interrupt.  */
        data_buffer[index + write_index] = (UCHAR)write_buffer[write_index];
        data_size = index + write_buffer_size;
    }
