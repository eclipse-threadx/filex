...
Check fat interrupt for fx_file_write() :                             
...
Step7: Terminate the new thread to simulate poweroff when write the fat between undo log generated and redo log not generated.  
...
void    filex_fault_tolerant_file_write_fat_interrupt_test_application_define(void *first_unused_memory);
...
{    
    ...
    /* Seek to the file. FAT 12, 16 and 32.  */
    if ((i % SEEK_COUNT) == 0)
    {
        status = fx_file_seek(&my_file, 0);
    }
