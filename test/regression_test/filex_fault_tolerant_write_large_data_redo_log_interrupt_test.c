...
Step7: Terminate the new thread to simulate poweroff when update the fat redo log.  
...
void    filex_fault_tolerant_write_large_data_redo_log_interrupt_test_application_define(void *first_unused_memory);
...
ULONG       data_value;
    ...
    printf("FileX Test:   Fault Tolerant Write Large Data Redo Log Interrupt test.");
...
static UINT my_driver_write(FX_MEDIA *media_ptr, UINT sector_type, UCHAR *block_ptr, UINT *operation_ptr)
{
    ...
    /* Interrupt the redo log write operation after record the undo log.  */
