...
void    filex_fault_tolerant_write_large_data_undo_log_interrupt_test_application_define(void *first_unused_memory);
...
static UINT                     undo_log_write_interrupt = FX_FALSE;    
static ULONG                    i, j;
...
ULONG       data_value;
    ...
    printf("FileX Test:   Fault Tolerant Write Large Data Undo Log Interrupt test.");
    ...
    for (i = 0; i < TEST_COUNT; i ++)
    {
        ...
        /* undo_log_write_interrupt */
        undo_log_write_interrupt = FX_FALSE;
        ...
        if ((status != FX_SUCCESS) || (undo_log_write_interrupt != FX_TRUE) || (error_couter))
        {
...
{           
    ...
    /* Interrupt the undo write operation.  */
    if ((sector_type == FX_DATA_SECTOR) && (_filex_fault_tolerant_log_check(media_ptr) & FX_FAULT_TOLERANT_LOG_UNDO_UPDATING))
    {
        ...
        *operation_ptr = FX_OP_WRITE_INTERRUPT;
        ...
        /* Update the flag.  */
        undo_log_write_interrupt = FX_TRUE;   
