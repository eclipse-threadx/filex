...
Step7: Terminate the new thread to simulate poweroff when cleanup the fat chain after redo log generated.  
...
void    filex_fault_tolerant_write_large_data_fat_chain_cleanup_interrupt_test_application_define(void *first_unused_memory);
...
static UINT                     fat_chain_cleanup_interrupt = FX_FALSE;    
static ULONG                    i, j;
...
ULONG       data_value;
    ...
    printf("FileX Test:   Fault Tolerant Write Large Data Fat Chain Cleanup Test.");
    ...
    for (i = 0; i < TEST_COUNT; i ++)
    {
        ...
        /* Write 2500 data to test file, take up two cluster.  */
        status =  fx_file_write(&my_file, (void *) large_data_buffer, 2500);
        ...
        fat_chain_cleanup_interrupt = FX_FALSE;
        ...
        status =  fx_media_space_available(&ram_disk, &j);
        {
            ...
            /* Check for available bytes error, two cluster.  */
            if ((status != FX_SUCCESS) || (j != 2048 * 2))
            {
        ...
        if ((status != FX_SUCCESS) || (fat_chain_cleanup_interrupt != FX_TRUE) || (error_couter))
        {
...
static UINT my_driver_write(FX_MEDIA *media_ptr, UINT sector_type, UCHAR *block_ptr, UINT *operation_ptr)
{
    ...
    /* Interrupt the FAT chain cleanup operation after record the redo log.  */
    if ((sector_type == FX_FAT_SECTOR) && (_filex_fault_tolerant_log_check(media_ptr) & FX_FAULT_TOLERANT_LOG_REDO_DONE))
    {
        ...
        *operation_ptr = FX_OP_WRITE_INTERRUPT;
        ...
        /* Update the flag.  */
        fat_chain_cleanup_interrupt = FX_TRUE;   
